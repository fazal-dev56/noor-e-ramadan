...
export const DuaSection: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    if (typeof data === 'string') {
      return (
        <div className="text-center h-full flex flex-col justify-center items-center p-2 md:p-4">
          ...
          <p className="text-sm md:text-xl leading-relaxed font-medium italic text-amber-800">"{data}"</p>
        </div>
      );
    }
    ...
    return (
      <div className="text-center space-y-1 md:space-y-4">
        <h3 className="text-xs md:text-xl font-bold text-amber-900 opacity-80 uppercase tracking-widest mb-1 md:mb-2">{data.title}</h3>
        <p className="text-xl md:text-4xl font-arabic leading-relaxed md:leading-loose text-amber-900 drop-shadow-sm py-1 md:py-2" dir="rtl">{data.arabic}</p>
        <div className="space-y-1 md:space-y-2 mt-2 md:mt-4 pt-2 md:pt-4 border-t border-amber-900/10">
            <p className="text-sm md:text-xl font-medium text-amber-800 font-serif" dir="rtl">{data.urdu}</p>
            <p className="text-[10px] md:text-base text-amber-700 italic leading-tight">{data.english}</p>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-0 md:px-4 pb-0 md:pb-20">
      
      {/* Tabs */}
      <div className="flex justify-between items-center gap-2 md:gap-4 mb-3 md:mb-6 relative px-4 md:px-0">
